...
lemma cutoff_eq_7:
  "real Y * 2 powr (-real s\<^sub>M) / b \<le> 1"
proof -
  have "real Y = 2 powr (log 2 Y)"
    using Y_ge_1 by (intro powr_log_cancel[symmetric]) auto
  also have "... \<le> 2 powr (nat \<lceil>log 2 Y\<rceil>)"
    by (intro powr_mono) linarith+
  also have "... = 2 ^ (nat \<lceil>log 2 Y\<rceil>)"
    by (subst powr_realpow) auto
  also have "... = real (2 ^ (nat \<lceil>log 2 (real Y)\<rceil>))"
    by simp
  also have "... \<le> real (2 ^ (b_exp + nat (\<lceil>log 2 (real Y)\<rceil> - int b_exp)))"
    by (intro Nat.of_nat_mono power_increasing) linarith+
  also have "... = b * 2^s\<^sub>M"
    unfolding s\<^sub>M_def b_def by (simp add: power_add)

  finally have "real Y \<le> b * 2 ^ s\<^sub>M" by simp
  thus ?thesis
    using b_min
...
proof -
  ...
  define Z where "Z \<omega> = (\<Sum>j<b. real_of_int \<lfloor>log 2 (of_int (max (\<tau>\<^sub>1 \<omega> A s\<^sub>M j) (-1)) + 2)\<rfloor>)" for \<omega>
  define V where "V \<omega> = Z \<omega> / real b - 3" for \<omega>

  have 2:"Z \<psi> \<le> real b*(real c+2) + of_int (\<Sum>a\<in>A. max 0 (int (fst \<psi> a) - s\<^sub>M -2^c))" 
  ...
  proof -
    ...
    have "- 1 - int s\<^sub>M - 2 ^ c \<le> -1 - 0 - 1"
      by (intro diff_mono) auto
    also have "... \<le> 0" by simp
    finally have "- 1 - int s\<^sub>M - 2 ^ c \<le> 0" by simp
    hence aux3_2: "max 0 (- 1 - int s\<^sub>M - 2 ^ c) = 0" 
      by (intro max_absorb1)

    have "?L1 \<le> (\<Sum>j<b. (real c+2) + max (real_of_int (max (\<tau>\<^sub>1 \<psi> A s\<^sub>M j) (- 1)) - 2^c) 0)"
      unfolding Z_def by (intro sum_mono cutoff_eq_5) auto 
    also have "... = (\<Sum>j<b. (real c+2)+max (\<tau>\<^sub>0 \<psi> A j - s\<^sub>M - 2^c) 0)"
      unfolding \<tau>\<^sub>1.simps max_of_mono[OF mono_real_of_int,symmetric] 
      by (intro_cong "[\<sigma>\<^sub>2 (+)]" more:sum.cong) (simp add:max_diff_distrib_left max.assoc aux3)
    also have "... = real b * (real c + 2) + 
      of_int (\<Sum>j<b. (max 0 (Max (insert (- 1) {int (f a) |a. a \<in> A \<and> h (g a) = j}) - s\<^sub>M - 2 ^ c)))"
      unfolding \<psi>_def by (simp add:max.commute)
    also have "... = real b * (real c + 2) + 
      of_int (\<Sum>j<b. max 0 (Max ((\<lambda>x. x-s\<^sub>M-2^c)`(insert(-1){int (f a) |a. a \<in> A \<and> h (g a) = j}))))"
      using fin_A
      by (intro_cong "[\<sigma>\<^sub>2 (+), \<sigma>\<^sub>1 of_int, \<sigma>\<^sub>2 max]" more:sum.cong mono_Max_commute) (auto simp:monoI)
    also have "... = real b * (real c + 2) + 
      of_int (\<Sum>j<b. max 0 (Max (insert(-1-s\<^sub>M-2^c){int (f a)-s\<^sub>M-2^c |a. a \<in> A \<and> h (g a) = j})))"
      by (intro_cong "[\<sigma>\<^sub>2 (+), \<sigma>\<^sub>1 of_int, \<sigma>\<^sub>2 max, \<sigma>\<^sub>1 Max]" more:sum.cong)  auto
    also have "... = real b * (real c + 2) + 
      of_int (\<Sum>j<b. Max ((max 0) `(insert(-1-s\<^sub>M-2^c){int (f a)-s\<^sub>M-2^c |a. a \<in> A \<and> h (g a) = j})))"
    ...
    also have "... = real b * (real c + 2) + 
      of_int (\<Sum>j<b. Max (insert 0 {max 0 (int (f a)-s\<^sub>M-2^c) |a. a \<in> A \<and> h (g a) = j}))"
    ...
    also have "... \<le> real b*(real c+2)+ of_int(\<Sum>j<b. (\<Sum>a|a\<in>A\<and>h(g(a)) =j. max 0(int(f a)-s\<^sub>M-2^c)))"
      using fin_A Max_le_Sum unfolding setcompr_eq_image
      by (intro add_mono iffD2[OF of_int_le_iff] sum_mono Max_le_Sum) (simp_all)
    also have "... = real b*(real c+2)+ 
      of_int(\<Sum>a\<in>(\<Union>j\<in>{..<b}. {a. a\<in>A\<and> h(g(a)) = j}). max 0(int(f a)-s\<^sub>M-2^c))"
      using fin_A
      by (intro_cong "[\<sigma>\<^sub>2 (+), \<sigma>\<^sub>1 of_int]" more:sum.UNION_disjoint[symmetric]) auto
    also have "... = real b*(real c+2) + of_int(\<Sum>a\<in>A. max 0(int(f a)-s\<^sub>M-2^c))"
  ...
  proof -
    ...
    also have "... = (\<Sum>a\<in>A. (\<integral>f.  of_int (max 0 (int (f a) - s\<^sub>M -2^c)) \<partial>(map_pmf fst \<Psi>)))/real b"
      by simp
    also have "... = (\<Sum>a\<in>A. (\<integral>f. of_int (max 0 (int (f a) - (s\<^sub>M +2^c))) \<partial>\<Psi>\<^sub>1))/real b"
      unfolding sample_pmf_\<Psi> map_fst_pair_pmf by (simp add:algebra_simps)
    also have "... \<le> (\<Sum>a\<in>A. 2 powr -real (s\<^sub>M + 2^c))/real b"
      using b_min by (intro sum_mono divide_right_mono cutoff_eq_6) auto
    also have "... = real Y * 2 powr (- real s\<^sub>M + (- (2 ^ c))) / real b" 
      unfolding Y_def by simp
    also have "... = (real Y * 2 powr (-real s\<^sub>M) / b) * 2 powr (-(2^c))"
