...
(* also something similar already exists in Set_Interval - see sum_telescope *)
...
text
\<open>A discrete probability space representing throwing |R| balls into |B| bins (k-wise independently)\<close>
...
(* 
  It makes much more sense to compute this in the fully independent case.
*)
lemma hit_count_factorial_moment:
  assumes "j \<in> B" "s \<le> k"
  shows "(\<integral>\<omega>. ffact s (Z j \<omega>) \<partial>p) = ffact s (real (card R)) / real (card B)^s" (is "?L = ?R")
proof -
  let ?r = "card R"
  define \<alpha> where "\<alpha> = (\<lambda>y. measure p {\<omega>. Z j \<omega> = y})"
  define \<beta> where "\<beta> = 1 / real (card B)" 

  have \<alpha>_eq: "\<alpha> y = (?r choose y) * \<beta>^y * (1-\<beta>)^y" for y
    sorry

  have "?L = (\<Sum>y\<le>?r. ffact s (real y) * \<alpha> y)"
    unfolding \<alpha>_def Z_def of_nat_eq_iff using fin_R card_mono
    by (intro measure_pmf.pmf_exp_of_fin_function[OF refl] finite_imageI image_subsetI)
     (auto intro!: card_mono)
  show ?thesis
    sorry
qed
...
lemma hit_count_moments:
  ...
  shows "(\<integral>\<omega>. Z j \<omega>^s  \<partial>p) = (\<Sum>xs \<in> L. (1/real (card B))^card (set xs))"
    (is "?L = ?R")
proof -
  let ?M = "{(xs,ys). set xs \<subseteq> R \<and> set ys \<subseteq> R \<and> 
    (set xs \<inter> set ys = {} \<or> j = j) \<and> length xs = s \<and> length ys = 0}"
...
proof -
  ...
  define L where "L = {(xs,ys). set xs \<subseteq> R \<and> set ys \<subseteq> R \<and> (set xs \<inter> set ys = {} \<or> j1 = j2) \<and> 
    length xs = s \<and> length ys = t}"
  define L' where "L' = {(xs,ys). set xs \<subseteq> R \<and> set ys \<subseteq> R \<and> length xs = s \<and> length ys = t}"
...
(*
  E ffact (k+1) (Z j1 + Z j2) 
  = E (Z j1 + Z j2) (Z j1 + Z j2 - 1) (Z j1 + Z j2... - (k-1))
  = E (sum_j=0..k+1. (k+1) choose j * ffact j (Z j1) * ffact (k+1-j) (Z j2))
  = sum_j=0..k+1. (k+1) choose j [E ffact j (Z j1) * ffact (k+1-j) (Z j2)]    
  = (Z j1) (Z j1-1) (Z j1-2) .. (Z j1 -j+1) (E Z j2) ...
 


  (* Bulwahn Falling Factorial Sum *)
...
proof -
  ...
  define fp :: "real poly" 
    where "fp = 1 - smult ((-1)^k / real (fact k)) (\<Prod>y \<in> I. [:- y, 1:])"
  ...
  have "(\<Prod>y \<in> I. (x - y)) = ffact k (x-1)" for x
  proof -
    have "(\<Prod>y \<in> I. (x - y)) = (\<Prod>y \<in> real ` ((\<lambda>x. x +1) ` {0..k-1}). (x - y))"
      unfolding I_def using k_ge_2 by (intro prod.cong) auto
    also have "... = (\<Prod>y \<in> (\<lambda>x. real x +1) ` {0..<k}. (x - y))"
      using k_ge_2 by (intro prod.cong, subst image_image, auto) 
    also have "... = (\<Prod>y \<in> {0..<k}. ((x - 1) - real y))"
      by (subst prod.reindex) (auto simp add:inj_on_def comp_def algebra_simps)
    also have "... = ffact k (x-1)"
      by (intro prod_ffact)
    finally show ?thesis by simp
  qed

  hence f_eval': "f x = 1 - (-1)^k / real (fact k) * ffact k (x-1)" for x
    unfolding f_eval by simp
  ...
  proof -
    ...
    also have "... = \<bar>(-1)^k / real (fact k) * (ffact k (x-2) - ffact k (x-1))\<bar>"
      unfolding f_eval' by (simp add:algebra_simps)
  ...
  have g_bound_abs: "\<bar>\<integral>\<omega>. g (f \<omega>) \<partial>p\<bar> \<le> (\<integral>\<omega>. ffact (k+1) (f \<omega>) \<partial>p) / fact k" 
    (is "?L \<le> ?R") 
  ...
  proof -
    ...
    also have "... \<le> (\<Sum>y\<in>{k+1..m}. \<bar>g (real y)- g (real y-1)\<bar> * 
      M.prob {\<omega>. ffact (k+1) (f \<omega>) \<ge> ffact (k+1) (real y)})" 
      apply (intro sum_mono mult_mono M.pmf_mono[OF M_def]) apply simp_all
      sorry
    ...
    also have "... \<le> (\<Sum>y\<in>{k+1..m}. (ffact (k-1) (real y-2) / fact (k-1)) * 
      (M.expectation (\<lambda>\<omega>. ffact (k+1) (f \<omega>)) / ffact (k+1) (real y)))"
      using k_ge_2 f_nat
      by (intro sum_mono mult_left_mono M.pmf_markov[OF M_def] f_any_integrable divide_nonneg_pos 
          ffact_of_nat_nonneg ffact_pos) auto
    also have "... = M.expectation (\<lambda>\<omega>. ffact (k+1) (f \<omega>)) / fact (k-1) * (\<Sum>y\<in>{k+1..m}.
    ...
    also have "... = M.expectation (\<lambda>\<omega>. ffact (k+1) (f \<omega>)) / fact (k-1) * 
    ...
    also have "... = M.expectation (\<lambda>\<omega>. ffact (k+1) (f \<omega>)) / fact (k-1) * (of_bool (k \<le> m) * (1/k-1/m))"
      by (subst sum_telescope_eq, auto)
    also have "... \<le> M.expectation (\<lambda>\<omega>. ffact (k+1) (f \<omega>)) / fact (k-1) * (1/ real k)" 
      using k_ge_2 f_nat
      by (intro mult_left_mono divide_nonneg_nonneg Bochner_Integration.integral_nonneg 
  ...
  proof -
    ...
    have "?L1 \<le> measure_pmf.expectation (p c) (\<lambda>\<omega>. ffact (k+1) (Z i \<omega>)) / fact k"
      unfolding Z_def using H.fin_R
      by (intro g_bound_abs[where m="card R"]) (auto intro!:imageI card_mono)
    also have "... \<le> real (card R) / real (card B) * Bell (k+1) / fact k"
      sorry
  ...
  proof -
    have a:"(\<integral>\<omega>. (Z i \<omega>^d * Z j \<omega>^(k+1-d)) \<partial>(p c)) \<le> Bell (k+1)" (is "?L c \<le> ?R") 
      if "d \<in> {..k+1}" for d
    proof -
    ...
    also have "... = (\<integral>\<omega>. (\<Sum>d \<le> k+1. real (k+1 choose d) * 
      (Z i \<omega>^d * Z j \<omega>^(k+1-d))) \<partial>(p c)) / real (fact k)"
      by (subst binomial_ring) (simp add:algebra_simps)
    also have "... = (\<Sum>d \<le> k+1. (\<integral>\<omega>. real (k+1 choose d) * 
      (Z i \<omega>^d * Z j \<omega>^((k+1)-d)) \<partial>(p c))) / real (fact k)"
    ...
    also have "... = (\<Sum>d \<le> k+1. real (k+1 choose d) * 
      (\<integral>\<omega>. (Z i \<omega>^d * Z j \<omega>^(k+1-d)) \<partial>(p c))) / real (fact k)"
  ...
  also have "... = \<bar>(\<Sum>i \<in> B \<times> B. 
    ...
    using Z_any_integrable Z_any_integrable' 
  ...
  also have "... = (2^(k+2)+4) * \<gamma> * card B^2"
    ...
    "\<bar>(\<integral>\<omega>. Y \<omega> ^2 \<partial>?p1) - (\<integral>\<omega>. Y \<omega>^2 \<partial>?p2)\<bar> \<le> (2^(k+2)+4) * \<gamma> * card B^2" 
    by simp
  ...
  also have "... = (2^(k+2)+2^3) * \<gamma> * card B^2"
    by (simp add:algebra_simps power2_eq_square)
  also have "... \<le> (2^(k+2)+2^(k+2)) * \<gamma> * card B^2"
    using k_ge_2 \<gamma>_nonneg
    by (intro mult_mono add_mono power_increasing, simp_all)
...
(*
  B_k+1 / k! \<le> (0.792e (1+1/k)/ln(k+2))^(k+1) (k/e) \<le> (2.4/ ln(k+2))^(k+1) (k/e)
  \<le> (3/ ln(k+2))^(k+1) \<le> (3/ln(k))^k 

  k \<ge> 20

  (k/e)^(1/[k+1]) = exp(ln(k/e)/k+1) \<le> exp ( ln(k+1)/k+1 ) \<le> exp ( ln(11)/11) \<le> 1.24


  (2^(k+2)+8) * B_k+1 / k! * b^2 \<le> 

  2^(k+3) * (3/ln(k))^k \<le> 8 * (6/ln(k))^k * b^2 \<le> eps^2

  (ln(k)/6)^k \<ge> 8 (1/eps)^2 * b^2

  k [ln ln k - 6] \<ge> ln (8) + 2 ln(1/eps) + 2 ln(b)
  ...
  k ln ln k \<ge> (x / ln x) * ln ln (x / ln x) = x / ln x * [ln ln x - ln ln ln x]


  \<le> 2^(k+3) * (0.792 (k+1) / ln(k+2))^(k+1) / (k/e)^k * b^2 
  \<le> 8/e * 2^(k+1) * e^k+1 * k * (0.792 (1+1/k) / ln(k+2))^(k+1) * b^2
  \<le> 8/e * k * [0.792 *2 *e * (1+1/k) / ln(k+2)]^(k+1) * b^2
  \<le> eps^2


 [0.792 *2 *e * (1+1/k) * exp( ln(k)/[k+1]) * (8/e)^(1/k+1) / ln(k+2)]^(k+1)
  \<le> [6/ ln(k+1)]^(k+1)
  \<le> [1/2]^(k+1)
  \<le> (eps/b)^2

  k \<ge> 2/0.69 [ln b + ln (eps^{-1})]
  k \<ge> C [ln b + ln(e^{-1})]
  k \<ge> C ln b
