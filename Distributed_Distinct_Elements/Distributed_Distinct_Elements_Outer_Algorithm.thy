...
definition stage_two where "stage_two = (\<epsilon> < (1/ln n\<^sub>0))"
definition \<epsilon>\<^sub>i :: real where "\<epsilon>\<^sub>i = (if stage_two then (1/ln n\<^sub>0) else \<epsilon>)"
definition m :: nat where "m = (if stage_two then nat \<lceil>4 * ln (1/ \<epsilon>)/ln (ln n\<^sub>0)\<rceil> else 1)"
definition \<alpha> where "\<alpha> = (if stage_two then (1/ln n\<^sub>0) else 1)"

lemma m_lbound:
  assumes "stage_two"
  shows "m \<ge> 4 * ln (1/ \<epsilon>)/ln(ln n\<^sub>0)"
proof -
  have "m = real (nat \<lceil>4 * ln (1 / \<epsilon>) / ln (ln n\<^sub>0)\<rceil>)"
    using assms unfolding m_def by simp
  also have "... \<ge> 4 * ln (1 / \<epsilon>) / ln (ln n\<^sub>0)"
    by linarith
...
proof (cases "stage_two")
  case True
  have "0 < 4 * ln (1/ \<epsilon>)/ln(ln n\<^sub>0)"
...
theorem estimate_result:
  ...
  shows "measure p {\<omega>. \<bar>estimate (\<nu> \<omega> A)- real (card A)\<bar> > \<delta> * real (card A)} \<le> \<epsilon>" (is "?L \<le> ?R")
proof (cases "stage_two")
  case True
...
next
  ...
  hence "?L = measure p {\<omega>. \<delta> * real (card A) < \<bar>I.estimate (\<nu> \<omega> A ! 0) - real (card A)\<bar>}"
...
lemma state_bit_count:
  "bit_count (encode_state (\<nu> \<omega> A)) \<le> state_space_usage (real n, \<delta>, \<epsilon>)" 
    (is "?L \<le> ?R")
proof -
  ...
  also have "... \<le> (\<Sum>x\<leftarrow>[0..<m]. ereal (2^36 *(ln (1/\<epsilon>\<^sub>i)+ 1)/\<delta>\<^sup>2 + log 2 (log 2 (real n) + 3)))"
    using I.state_bit_count by (intro sum_list_mono I.state_bit_count \<Theta>.range)
  also have "... = ereal ( real m * (2^36 *(ln (1/\<epsilon>\<^sub>i)+ 1)/\<delta>\<^sup>2 + log 2 (log 2 (real n) + 3)))"
    unfolding sum_list_triv_ereal by simp
  also have "... \<le> 2^40 * (ln(1/\<epsilon>)+1)/ \<delta>^2 + log 2 (log 2 n + 3)" (is "?L1 \<le> ?R1")
  proof (cases "stage_two")
    case True
    have "\<lceil>4*ln (1/\<epsilon>)/ln(ln n\<^sub>0)\<rceil> \<le> 4*ln (1/\<epsilon>)/ln(ln n\<^sub>0) + 1"
      by simp
    also have "... \<le> 4*ln (1/\<epsilon>)/ln(ln n\<^sub>0) + ln (1/\<epsilon>)/ln(ln n\<^sub>0)"
      using m_gt_0_aux[OF True] by (intro add_mono) auto
    also have "... = 5 * ln (1/\<epsilon>)/ln(ln n\<^sub>0)" by simp
    finally have 3: "\<lceil>4*ln (1/\<epsilon>)/ln(ln n\<^sub>0)\<rceil> \<le> 5 * ln (1/\<epsilon>)/ln(ln n\<^sub>0)"
      by simp
    ...
    have "?L1 = real(nat \<lceil>4*ln (1/\<epsilon>)/ln(ln n\<^sub>0)\<rceil>)*(2^36*(ln (ln n\<^sub>0)+1)/\<delta>^2+log 2(log 2 (real n)+3))"
      using True unfolding m_def \<epsilon>\<^sub>i_def by simp
    also have "... = \<lceil>4*ln (1/\<epsilon>)/ln(ln n\<^sub>0)\<rceil>*(2^36*(ln (ln n\<^sub>0)+1)/\<delta>^2+log 2(log 2 (real n)+3))"
      using m_gt_0_aux[OF True] by (subst of_nat_nat) simp_all
    also have "... \<le> (5*ln (1/\<epsilon>)/ln(ln n\<^sub>0)) *(2^36*(ln (ln n\<^sub>0)+1)/\<delta>^2+log 2(log 2 (real n)+3))"
    ...
    also have "... \<le> (5 * ln (1/\<epsilon>)/ln(ln n\<^sub>0))*((2^36+2^36)*ln (ln n\<^sub>0)/\<delta>^2+log 2(log 2 (real n)+3))"
    ...
    also have "... = 5*(2^37)* ln (1/\<epsilon>)/ \<delta>^2 + (5*ln (1/\<epsilon>)) * (log 2(log 2 (real n)+3)/ln(ln n\<^sub>0))"
  ...
  next
    ...
    have "?L1 = 2^36 *(ln (1/\<epsilon>)+ 1)/\<delta>\<^sup>2 + log 2 (log 2 (real n) + 3)"
      using False unfolding \<epsilon>\<^sub>i_def m_def by simp
    also have "... \<le> ?R1"
      using \<delta>_gt_0 \<delta>_lt_1 \<epsilon>_gt_0 \<epsilon>_lt_1
...
proof -
  ...
  also have "...=1+4*log 2 n+48*(log 2(1/\<delta>)+16)\<^sup>2+(55+60*ln (1/\<epsilon>\<^sub>i))^3+(4*(m-1)*nat\<lceil>ln \<alpha>/ln 0.95\<rceil>)"
  ...
  proof (cases "stage_two")
    ...
    also have "... = 4 * (real_of_int \<lceil>4 * ln (1 / \<epsilon>) / ln (ln n\<^sub>0)\<rceil>-1) *  (ln (ln n\<^sub>0) * 20 + 1)"
      using m_gt_0_aux[OF True] by (subst of_nat_nat) simp_all
    also have "... \<le> 4 * (4 * ln (1 / \<epsilon>) / ln (ln n\<^sub>0)) * (ln (ln n\<^sub>0) * 20 + 1)"
      using n_lbound by (intro mult_left_mono mult_right_mono) auto
    also have "... \<le> 4 * (4 * ln (1 / \<epsilon>) / ln (ln n\<^sub>0)) * (ln (ln n\<^sub>0) * 20 + ln (ln n\<^sub>0))"
  ...
  next
    ...
    have "?L1 = 1 + 4 * log 2 (real n) + 48 * (log 2 (1 / \<delta>) + 16)\<^sup>2 + (55 + 60 * ln (1 / \<epsilon>)) ^ 3"
      using False unfolding \<epsilon>\<^sub>i_def m_def by simp
    also have "... \<le> 1 + 4 * log 2 (real n) + 48 * (log 2 (1 / \<delta>) + 16)\<^sup>2 + (55 + 60 * ln (ln n\<^sub>0))^3"
...
proof -
  ...
  also have "... = O[F](\<lambda>(n, \<delta>, \<epsilon>). ln (1/\<epsilon>)/\<delta>^2 + ln n)"
